...
public class StoryDataClientImplTests {
	...
	private static final IssueLinkType issuelinkType = new IssueLinkType("Praveen", "is cloned by", IssueLinkType.Direction.INBOUND);
	private static final IssueLink issuelink = new IssueLink("ST6-95", URI.create("https://my.jira.com/rest/api/2/issue/3"), issuelinkType);
	...
	public void testUpdateStoryInformation_NoPage() {
		...
		for ( FeatureIssueLink featureIssueLink:feature1.getIssueLinks()) {
			assertEquals("Praveen",featureIssueLink.getIssueLinkName());
			assertEquals("is cloned by",featureIssueLink.getIssueLinkType());
			assertEquals("ST6-95", featureIssueLink.getTargetIssueKey());
		...
		assertNotEquals(2, feature1.getIssueLinks().size());
	}
